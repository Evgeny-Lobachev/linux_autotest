...
class TestPositive:
    ...
    def test_0(self, start_time):
        ...
        res.append(ssh_checkout(data['host'], data['user'], data['password'], "echo '1111' | sudo -S dpkg -i p7zip.deb",
                                "Настраивается пакет"))
    ...
    def test_9(self, start_time):
        res = []
        res.append(ssh_checkout(data['host'], data['user'], data['password'],
                                "echo '1111' | sudo -S dpkg -r p7zip-full", "Удаляется p7zip-full"))
        res.append(ssh_checkout(data['host'], data['user'], data['password'],
                                "echo '1111' | sudo -S dpkg -s p7zip-full", "Status: deinstall ok"))
        self.save_log(start_time, 'test_log.txt')
