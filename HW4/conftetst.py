...
def make_files():
    ...
    for i in range(data['count']):
        ...
        if ssh_checkout(data['host'], data['user'], data['password'],
                        f"cd {data['folder_tst']}; dd if=/dev/urandom of={filename} bs={data['bs']} "
...
def make_sub_folders():
    if ssh_checkout(data['host'], data['user'], data['password'], f"mkdir {data['folder_tst']}/{data['sub_folder']} ",
                    ""):
        if ssh_checkout(data['host'], data['user'], data['password'],
                        f"cd {data['folder_tst']}/{data['sub_folder']}; dd if=/dev/urandom of=sub_file bs={data['bs']} "
                        f"count=1 iflag=fullblock", ""):
...
@pytest.fixture()
def make_bad_files():
    ssh_checkout(data['host'], data['user'], data['password'], f"cd {data['folder_tst']}; "
                                                               f"7z a {data['folder_out']}/arx2bad", "Everything is Ok")
...
def save_stat():
    ...
    stat_str = f"{datetime.now()} - количество файлов: {data['count']}, размера: {data['bs']}, " \
               f"статистика процессора: {static_proc}"
